<div class="client_name"><b>Клиент:</b> <span class="value">{{$consultations->client_name}}</span> </div>
<br>
<form action="/applications/conclusion_post/{{$consultations->id}}" method="post">
   {{csrf_field()}}
   <input type="hidden" name="consultation" id="application_id">
   <div class="row">
      <div class="col">
         <b>Заключение:</b>
         <textarea name="conclusion" class="form-control" id="conclusion" @if($consultations->status_id == 4) readonly @endif>{{$consultations->conclusion}}</textarea>
         </div>
      </div>
      <br>
               ...
               @if(json_decode($consultations->npa) == null)
                  <div id="field0" name="field0">
                     <div class="form-group">
                        <input id="npa" name="npa[]" type="text" placeholder="Ссылка" class="form-control input-md" @if($consultations->status_id == 4) readonly @endif>
                           <br>
                        </div>
                     </div>
                  @else
                     @foreach (json_decode($consultations->npa) as $key => $value)
                        @if($value != '')
                           @if($consultations->status_id != 4)
                              <div id="field{{$key}}" name="field{{$key}}">
                                 <div class="form-group">
                                    <input id="npa" name="npa[]" type="text" placeholder="Ссылка" class="form-control input-md" value="{{$value}}" @if($consultations->status_id == 4) readonly @endif>
                                       <br>
                                    </div>
                                 </div>
